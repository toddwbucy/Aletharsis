...
The script builds with `go build -trimpath` and installs with `go install -trimpath ./cmd/aletharsis` into a new temporary absolute `GOBIN`. It executes both resulting binaries directly with an empty PATH, checks help/version, and audits disposable generated fixtures twice in every view. It never selects an auditor from PATH or imports the Python auditor. All build/install invocations have a 180-second timeout; each candidate invocation has a 10-second timeout. Ordinary failures surface with diagnostics in the job log; the completion artifact is written only after all checks succeed.

On Linux, 24 built/installed cases cover clean ASCII, supplementary emoji/ZWSP/CRLF coordinates, invalid encoding, source digests, deterministic output, process exit statuses and live-schema validation. The larger independent integration/parity suites cover additional detection and failure contracts.
