...
Defaults allow at most 8 MiB of source and 16 MiB of serialized report, with 4,194,304 report nodes and depth 64. The node allowance accommodates the million-plus boundary entries in a 1 MiB ASCII input while retaining the 16 MiB byte bound. Callers may lower the source bound and set independent report budgets. Invalid options fail before acquisition. Finding ordering keys use the whole-report budget because finding payloads are report members, not standalone execution/anchor records. Anchor and selection records retain their existing 1 MiB, 32,768-node and depth-32 limits. Disjoint selections are partitioned into at most 512 spans per anchor. Adjacent observations coalesce up to 16,384 scalars; an individual token extent is never cut merely to fit this grouping target. A finding can reference multiple anchors without losing any occurrence. A single extent exceeding the selection limit still fails explicitly. A source accepted by the legacy pipeline may exceed v2 evidence budgets. Such failures return an error and no purported complete report; evidence is not silently truncated.

These are validation/serialization budgets, not hard process-memory ceilings. Native evidence and marshaled ordering keys are allocated before some checks. A post-parse lower-bound check counts literal text bytes and mandatory decimal boundary entries before analysis; impossible report byte/node budgets fail before analyzer and anchor allocations. Graph validation reuses text/UTF-8 projections within one validation call. Full validation still runs, and the cache is not retained across mutable consumer reports. Large-report measurement remains required before release acceptance.
