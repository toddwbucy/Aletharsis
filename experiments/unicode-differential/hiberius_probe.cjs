...
class Element {
  constructor() { this.children = []; this.events = {}; this.value = ''; this.style = {}; this.hidden = false; this._text = ''; }
...
vm.runInNewContext(scripts[0][1], {document, TextEncoder, TextDecoder}, {timeout:1000});
const request = JSON.parse(fs.readFileSync(0,'utf8'));
get('scanInput').value = request.text;
get('btnScan').events.click();
...
if (request.text && scalar !== input.length) throw new Error('visual map incomplete');
process.stdout.write(JSON.stringify({position_units:'Unicode scalar indices reconstructed from scanViz child order',
  observations, verdict:get('scanVerdict').textContent, scan_status:request.text ? 'completed' : 'empty_input_not_scanned', secret_visible:request.text ? !get('scanSecret').hidden : null,
