...
func TestWholeScopeLimitOmission(t *testing.T) {
	...
	source := []byte(document(`<w:p>` + run(payload) + `</w:p><w:p>` + run("z") + `</w:p>`))
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			o := got.Omitted[0]
			if o.LocalID != "word-scope/0" || o.Dimension != tc.dimension || o.Code != failure.ResourceLimit || o.TextUTF8Bytes != bytes || o.ScalarOrigins != scalars {
				t.Fatal(o)
			}
...
func TestScopeLimitCountsDecodedCrossRunText(t *testing.T) {
	source := []byte(document(`<w:p>` + run(`&#x1F600;`) + `<w:r><w:rPr><w:b/></w:rPr><w:t>&#x200B;</w:t></w:r></w:p><w:p>` + run("z") + `</w:p>`))
	got, err := AnalyzeWithLimits(context.Background(), source, evidence.Hash(source), scopelimits.Limits{TextUTF8Bytes: 6, ScalarOrigins: 10})
	if err != nil {
		t.Fatal(err)
	}
	if len(got.Omitted) != 1 || got.Omitted[0].ScalarOrigins != 2 || got.Omitted[0].TextUTF8Bytes != 7 || len(got.Scopes) != 1 || got.Scopes[0].Text != "z" {
		t.Fatal("decoded cross-run bytes escaped cap", got)
	}
}
