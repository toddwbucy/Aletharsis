...
func TestOrdinaryLocatedFindingsDoNotReplaceProse(t *testing.T) {
	...
	if err != nil || got.Text != string(raw) || len(got.Occurrences) != 1 {
		t.Fatal("ordinary prose replaced", err)
	}
	occurrence := got.Occurrences[0]
	if occurrence.Reason != "finding_reference" || len(occurrence.Findings) != 1 || occurrence.Findings[0].ID != f.ID || occurrence.SourceBytes.Start != 0 || occurrence.SourceBytes.End != 1 || occurrence.RenderedBytes != occurrence.SourceBytes {
		t.Fatal("ordinary finding lost its unchanged coordinate mapping")
	}
}
...
func TestNativePrintableFindingsRetainOccurrenceReferences(t *testing.T) {
	raw := []byte("document-id: 123e4567-e89b-12d3-a456-426614174000 generated by test")
	d, err := (parsers.TextParser{}).Parse(raw)
	if err != nil {
		t.Fatal(err)
	}
	findings := (analyzers.Identifiers{}).Analyze(&d)
	got, err := Render(raw, evidence.Hash(raw), d.Texts[0], findings, testLimits)
	if err != nil || got.Text != string(raw) {
		t.Fatal("printable text modified", err)
	}
	linked := map[int]bool{}
	for _, o := range got.Occurrences {
		if o.Reason != "finding_reference" || o.SourceBytes != o.RenderedBytes {
			t.Fatal("incorrect identity map", o)
		}
		for _, f := range o.Findings {
			linked[f.Index] = true
		}
	}
	if len(findings) < 2 || len(linked) != len(findings) {
		t.Fatal("native finding links lost", len(findings), linked)
	}
}
