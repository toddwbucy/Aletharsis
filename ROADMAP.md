...
| **P2 — Format profiles and office containers** — *partially delivered* | DOCX first, then a separately scoped ODT increment; contextual expected-artifact profiles; [PA-001 standalone evaluator](docs/specs/expected-artifact-profiles-v1.md) | Evidence layer delivered in [#55](https://github.com/toddwbucy/Aletharsis/pull/55): `packageparts`, `xmlparts`, `opcrels`, `docxidentify`, `odtidentify`, `wordtext`, `odttext`, `wordanalysis`, `odtanalysis`, `profiles` and the `profile-v1` contract. Integration contract accepted in [#74](https://github.com/toddwbucy/Aletharsis/pull/74) as [OC-001](docs/specs/office-cli-evidence.md) with [ADR-0003](docs/adr/0003-office-report-contract.md): opt-in report 4.0, corpus-v2, reveal-tree-v2, three coordinate identities, budget tiering, per-version exit codes. Document metadata producer delivered in [#76](https://github.com/toddwbucy/Aletharsis/pull/76) (`internal/officemetadata`, full ECMA-376 core/app vocabulary). **Remaining:** the wire increment that implements OC-001 — `internal/audit` still dispatches only `text`, so no Office evidence reaches a report or the CLI yet; the embedded-object inventory has no producer; profile *content* for real formats is unwritten. [Issue #14](https://github.com/toddwbucy/Aletharsis/issues/14) tracks profiles. |
...
P2 integration is specified by the accepted [OC-001](docs/specs/office-cli-evidence.md):
opt-in report 4.0 with explicit Office coordinates and corpus-v2, preserving 1.0/2.0.
Implementation of that contract is the next P2 increment.
This is under review in [PR #74](https://github.com/toddwbucy/Aletharsis/pull/74),
not delivered CLI support.
The version decision is recorded in [ADR-0003](docs/adr/0003-office-report-contract.md).
